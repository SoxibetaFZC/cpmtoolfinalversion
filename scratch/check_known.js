...
async function checkKnownColumns() {
  const columnsToTry = ['emp_achievements', 'emp_blockers', 'emp_proof_points', 'manager_id', 'overall_result'];
  
  for (const col of columnsToTry) {
    const { error } = await supabase.from('monthly_reviews').select(col).limit(1);
    if (!error) {
