...
async function run() {
  try {
    ...
    if (error) console.log('Error fetching main_themes:', error);
    else console.log('Main Themes data:', JSON.stringify(data, null, 2));
  } catch (e) {
    console.log('Exception fetching main_themes:', e);
  }
}
run();
