...
async function checkLearning() {
  const { error } = await supabase.from('monthly_reviews').select('emp_learning').limit(1);
  if (!error) {
    console.log(`✅ Column 'emp_learning' EXISTS`);
  } else {
    console.log(`❌ Column 'emp_learning' DOES NOT exist (${error.message})`);
  }
}
